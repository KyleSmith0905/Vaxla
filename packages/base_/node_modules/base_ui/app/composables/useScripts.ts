...
const getScriptIndex = (id: string) =>
  scripts.value.findIndex((e) => e.id === id);
...
export default () => {
  const runScript = async (command: string, id?: string) => {
    id ??= nanoid();
  ...
  const killScript = async (id: string | undefined) => {
    ...
    await $fetch("/api/kill-script", { method: "POST", body: { id } }).catch(
      (error) => {
